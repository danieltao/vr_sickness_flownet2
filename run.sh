    ...
    for theta in {-180..180..15}
        do  
            ...
            srun -u python main.py --inference --model FlowNet2 --save_flow --inference_dataset ImagesFromFolder \
                        ...
                        --resume FlowNet2_checkpoint.pth.tar --save "/usr/xtmp/ct214/daml/vr_sickness/test" --name "left_eye_theta_${theta}_phi_${phi}"
            srun -u python main.py --inference --model FlowNet2 --save_flow --inference_dataset ImagesFromFolder \
                        --inference_dataset_root "/usr/xtmp/ct214/daml/vr_sickness/perspectives_skyhouse/right_eye/theta_${theta}_phi_${phi}/" \
                        --resume FlowNet2_checkpoint.pth.tar --save "/usr/xtmp/ct214/daml/vr_sickness/test" --name "right_eye_theta_${theta}_phi_${phi}"
        done
    done
